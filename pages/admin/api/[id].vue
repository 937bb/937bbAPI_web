    ...
    <form v-else class="api-detail-form" @submit.prevent="submitEdit">
      ...
      <div class="form-row">
        ...
        >
          ...
          <hr
            class="param-block-divider"
            v-if="
              form.requestParams.length > 1 &&
              idx < form.requestParams.length - 1
            "
          />
        </div>
        <button
      ...
      <div class="form-row">
        ...
        >
          ...
          <div class="param-block-row">
            <input v-model="field.type" placeholder="类型，如 string/number/object/array" />
          </div>
          <div class="param-block-row">
          ...
          <hr
            class="param-block-divider"
            v-if="form.responseStatus.length > 1 && idx < form.responseStatus.length - 1"
          />
        </div>
        <button
          type="button"
          class="edit-btn"
          @click="form.responseStatus.push({ name: '', type: '', meaning: '', description: '' })"
        >
          +字段
        </button>
      </div>
      ...
      <div class="modal-group">
        ...
        <div class="modal-group-list">
          <div v-for="g in groupOptions" :key="g" class="modal-group-item">
            <span>{{ g }}</span>
            <button class="edit-btn delete" @click="handleDeleteGroup(g)">
              删除
            </button>
          </div>
        </div>
        ...
        <div class="modal-actions">
          <button class="edit-btn cancel" @click="showGroupModal = false">
            关闭
          </button>
        </div>
      </div>
...
async function fetchGroups() {
  ...
  try {
    ...
    if (res.code === 200 && Array.isArray(res.data)) {
      groupOptions.value = res.data.map((g) => g.title);
    } else {
      groupOptions.value = [];
    }
  } catch {
...
async function handleAddGroup() {
  ...
  const token = user?.token || "";
  try {
    const res = await addGroup(name, token);
    if (res.code === 200) {
      newGroupName.value = "";
      await fetchGroups();
    } else {
