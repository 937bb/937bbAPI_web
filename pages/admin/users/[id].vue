<template>
  <div class="py-6 px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          >
            <path
              ...
              stroke-width="2"
              d="M11 17l-5-5m0 0l5-5m-5 5h12a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2z"
            />
          </svg>
          返回用户列表
        </button>
    ...
    <form
      ...
      class="space-y-6 bg-white shadow overflow-hidden rounded-lg"
    >
...
async function submitEdit() {
  ...
  try {
    ...
    showMessage(
      err.message || (isNew ? "创建用户失败" : "更新用户信息失败"),
      "error"
    );
  } finally {
    saving.value = false;
...
onMounted(() => {
  if (!isNew) {
    ...
    // Initialize form with default values for new user
    form.role = "user";
    form.status = 0;
  }
});
