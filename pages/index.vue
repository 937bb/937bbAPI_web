	...
	<div class="main-bg">
		...
		<div class="main-content">
			...
			<nav class="group-tabs" v-if="groupNames.length > 0">
				<div :class="['group-tab', { active: activeGroup === '' }]" @click="activeGroup = ''">全部</div>
				<div v-for="g in groupNames" :key="g.id" :class="['group-tab', { active: g.id === activeGroup }]" @click="activeGroup = g.id">
					{{ g.title }}
				</div>
			</nav>
			<!-- API卡片区 -->
...
const filteredApis = computed(() => {
	let list = apis.value;
	if (activeGroup.value) list = list.filter((a) => a.groupId === activeGroup.value);
